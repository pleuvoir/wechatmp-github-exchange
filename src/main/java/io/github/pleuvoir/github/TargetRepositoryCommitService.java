...
import io.github.pleuvoir.kit.ExchangeUtil;
...
public class TargetRepositoryCommitService implements CommitService {
	...
	public void commit(String content) {
		...
		// 检查文件是否存在
		try {
			GHContent fileContent = repository.getFileContent(ExchangeUtil.encodeUTF8(todayFilePath));
	...
	private void commitFile(byte[] bytes, String path, String sha) {
		...
		contentBuilder.sha(sha).content(bytes)
				...
				.path(ExchangeUtil.encodeUTF8(path)).commit();
	}
